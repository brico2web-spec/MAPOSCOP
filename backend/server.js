...
app.post("/api/auth/verify",async(req,res)=>{
 try{
  ...
  res.cookie("zamzam_session",tokenFor(user),{httpOnly:true,sameSite:process.env.NODE_ENV==="production"?"none":"lax",secure:process.env.NODE_ENV==="production",maxAge:7*24*3600*1000});
  res.json({ok:true});
...
async function ensureAdmin(){
 ...
 const email=norm(process.env.ADMIN_EMAIL).toLowerCase();
 if(!username || password.length<8) return;
 const exists=db.prepare("SELECT id FROM users WHERE username=?").get(username);
 if(!exists){
  const hash=await bcrypt.hash(password,12);
  db.prepare("INSERT INTO users(username,email,password_hash,verified,is_admin) VALUES(?,?,?,?,1)").run(username,email||null,hash,1);
  console.log("Admin account created:",username);
 }
}

app.get("/api/health",(req,res)=>res.json({ok:true,service:"ZAMZAM API"}));

ensureAdmin().then(()=>app.listen(PORT,()=>console.log(`ZAMZAM Store API running on port ${PORT}`))).catch(err=>{console.error(err);process.exit(1);});
